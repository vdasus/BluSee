...
public sealed class HidppClient(HidppTransport transport)
{
    ...
    public async Task<IReadOnlyList<HidppBatteryReading>> ReadAllAsync(CancellationToken ct)
    {
        ...
        for (byte index = 1; index <= 6; index++)
        {
            ...
            // Cheap connectivity gate: skip indices that don't answer root.getFeature at all.
            if (await GetFeatureIndexAsync(index, FeatureSet, ct) is null)
                continue;
    ...
    {
        var frame = LongFrame(deviceIndex, RootFeatureIndex, funcId: 0x00, 0x00, 0x01);
        return transport.RequestAsync(frame, r => r.Length >= 4 && r[1] == deviceIndex && (
            (r[2] == RootFeatureIndex && (r[3] & 0x0F) == SoftwareId) ||
            (r[2] == ErrorIndex && r[3] == RootFeatureIndex)), Timeout, ct);
    }
    ...
    {
        var frame = LongFrame(deviceIndex, RootFeatureIndex, funcId: 0x00,
            (byte)(featureId >> 8), (byte)(featureId & 0xFF));
        return transport.RequestAsync(frame, r => Matches(r, deviceIndex, RootFeatureIndex), Timeout, ct);
    }
    ...
    public Task<byte[]?> DebugCallAsync(byte deviceIndex, byte featureIndex, byte funcId, CancellationToken ct)
    {
        ...
        return transport.RequestAsync(frame, r => Matches(r, deviceIndex, featureIndex), Timeout, ct);
    }
    ...
    private async Task<byte?> GetFeatureIndexAsync(byte deviceIndex, ushort featureId, CancellationToken ct)
    {
        ...
        var reply = await transport.RequestAsync(frame, r => Matches(r, deviceIndex, RootFeatureIndex), Timeout, ct);
    ...
    private static byte[] LongFrame(byte deviceIndex, byte featureIndex, byte funcId, byte p0 = 0, byte p1 = 0)
    {
        ...
        frame[3] = (byte)((funcId << 4) | SoftwareId);
        frame[4] = p0;
        frame[5] = p1;
        return frame;
    }
    ...
    private static bool Matches(byte[] r, byte deviceIndex, byte featureIndex)
    {
        if (r.Length < 5 || r[1] != deviceIndex)
            return false;

        // normal: [.. , featureIndex, (funcId<<4)|swId, ..]
        if (r[2] == featureIndex && (r[3] & 0x0F) == SoftwareId)
            return true;

        // HID++ 2.0 error: [.. , 0xFF, failedFeatureIndex, (funcId<<4)|swId, errorCode]
        return r[2] == ErrorIndex && r[3] == featureIndex && r.Length >= 6 && (r[4] & 0x0F) == SoftwareId;
    }
